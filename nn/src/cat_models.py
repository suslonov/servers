  ...
  def __init__(self, config, input_data, labels, word_dict, name=None, shuffle=True):
    ...
    with tf.device("/cpu:0"):
      with tf.name_scope(name):
        ...
        labels_ = tf.string_to_number(labels, name="labels", out_type=tf.int32)
        x_len_ = tf.convert_to_tensor(x_len, dtype=tf.int32, name="x_len")
  ...
  def __init__(self, config, input_data, labels, word_dict, name=None, shuffle=True):
    ...
    with tf.device("/cpu:0"):
      with tf.name_scope(name):
        ...
        i = tf.train.range_input_producer(batch_len, shuffle=shuffle).dequeue()
...
class SLSTMCharModel(object):
  def __init__(self, is_training, config, input_, type_cell, data_type=tf.float32):
    ...
    def lstm_cell(type_cell):
      if type_cell == 'gru':
        return tf.nn.rnn_cell.GRUCell(hidden_size, reuse=tf.get_variable_scope().reuse)
      else:
        return tf.nn.rnn_cell.BasicLSTMCell(hidden_size, reuse=tf.get_variable_scope().reuse)

    attn_cell = lstm_cell
    if is_training and config.keep_prob < 1:
      def attn_cell(type_cell):
        return tf.nn.rnn_cell.DropoutWrapper(lstm_cell(type_cell), output_keep_prob=config.keep_prob)

    cell = tf.nn.rnn_cell.MultiRNNCell([attn_cell(type_cell) for _ in range(config.num_layers)], state_is_tuple=True)
    self._initial_state = cell.zero_state(batch_size, data_type)

    embedding = tf.get_variable("embedding", [vocab_size, hidden_size], dtype=data_type)
    inputs = tf.nn.embedding_lookup(embedding, input_.input_data)  # [batch_size, doc_len, hidden_size]
    ...
    with tf.variable_scope("RNN"):
      for step in range(doc_len):
        ...
        if step == 0:
          ...
          output = tf.where(tf.not_equal(input_.input_data[:,step], 0), cell_output, output)
        
#      output is [batch_size, hidden_size]

    softmax_w = tf.get_variable("softmax_w", [hidden_size, num_labels], dtype=data_type)
    ...
    loss = tf.losses.softmax_cross_entropy(onehot_labels=onehot_targets, logits=logits)
    ...
    if not is_training:
      _y = tf.argmax(logits, 2, output_type=tf.int32)
      c_p = tf.equal(_y, input_.targets)
      self._correct =  tf.reduce_sum(tf.cast(c_p, tf.float32))
      return
    else:
    ...
    optimizer = tf.train.GradientDescentOptimizer(self._lr)    #  try others
...
class DLSTMCharModel(object):
  def __init__(self, is_training, config, input_, type_cell, data_type=tf.float32):
    ...
    outputs, _ = tf.nn.dynamic_rnn(cell, inputs, dtype=data_type)   # sequence_length= input_.lens, 
...
class BiLSTMCharModel(object):
  def __init__(self, is_training, config, input_, type_cell, data_type=tf.float32):
    ...
    outputs, _ = tf.nn.bidirectional_dynamic_rnn(cell, cell, inputs, sequence_length= input_.lens,dtype=data_type)
    
    indices = tf.stack([tf.range(batch_size), tf.subtract(input_.lens, 1)], 1)
    output_ = tf.concat([outputs[0], outputs[1]], 2)
    output = tf.gather_nd(output_, indices) #      output is [batch_size, hidden_size]

    softmax_w = tf.get_variable("softmax_w", [hidden_size*2, num_labels], dtype=data_type)
    softmax_b = tf.get_variable("softmax_b", [num_labels], dtype=data_type)
    logits = tf.matmul(output, softmax_w) + softmax_b
