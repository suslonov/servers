...
class CotiRocksUsrDB(object):
    ...
    def __init__(self, dbname: str, options=default_db_options()):
    ...
    def read_usr_parms_from_db(self, user_ID: str):
        ...
        if user_info == "None":
            ...
            for parm in parms_array:
                user_parameters[parms.ordered_user_parms[counter]] = parm
                counter += 1
            return user_parameters

    #

    def update_write_user_parms_to_db(self, user_params: dict, allow_updating=True):
        user_id = user_params["user_ID"]
        user_info = self.__get(user_id)
        if (not allow_updating) and (user_info != "None"):
            raise Exception(
                "Tried to update a user_ID in database when the option for this was expressly forbidden.")

        # complementary data to make up for any missing values etc.
        complementary_data = dict()
        if len(user_params) < len(parms.ordered_user_parms):
            complementary_data = parms.default_user if user_info == "None"\
                else self.read_usr_parms_from_db(user_id)
        user_info = ""
        counter = 0
        for usr_parm in parms.ordered_user_parms:
            if usr_parm == "user_ID":
                continue
            #
            if counter != 0:
...
class CotiRocksTxnDB(object):
    ...
    def __init__(self, dbname: str, db_id_by="sender_ID", options="default"):
        if options == "default":
